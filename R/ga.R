...
ga <- function(
  ...
  callArgs <- list(...)
  restart <- "object" %in% names(callArgs) && inherits(callArgs$object, "ga")
  ...
  if (missing(lower) & missing(upper) & missing(nBits)) {
    stop(
      "A lower and upper range of values (for 'real-valued' or 'permutation' GA) or nBits (for 'binary' GA) must be provided!"
    )
  }
  ...
  if (is.null(suggestions)) {
    ...
    if (is.vector(suggestions)) {
      if (nvars > 1) suggestions <- matrix(suggestions, nrow = 1) else
  ...
  if (optim) {
    ...
    if (is.null(optimArgs$control$fnscale)) optimArgs$control$fnscale <- -1
  ...
  `%DO%` <- if (parallel && requireNamespace("doRNG", quietly = TRUE))
    doRNG::`%dorng%` else if (parallel) `%dopar%` else `%do%`
  ...
  bestSol <- if (keepBest) vector(mode = "list", length = maxiter) else list()
  Fitness <- rep(NA, popSize)
  if (restart) {
    object <- callArgs$object
    restart_iter <- object@iter
  ...
  for (iter in seq_len(maxiter)) {
    ...
    if (!parallel) {
      ...
      Fitness <- foreach(i. = seq_len(popSize), .combine = "c") %DO%
        {
          if (is.na(Fitness[i.]))
            do.call(fitness, c(list(Pop[i., ]), callArgs)) else Fitness[i.]
        }
    }
    # update object
    object@population <- Pop
    ...
    if (keepBest) {
      object@bestSol[[object@iter]] <- unique(Pop[
        Fitness == max(Fitness, na.rm = TRUE),
        ,
        drop = FALSE
      ])
    }
    ...
    if (object@iter > 1) object@run <- garun(fitnessSummary[seq(object@iter), 1])
...
summary.ga <- function(object, ...) {
  ...
  if (object@type == "real-valued") {
    ...
    if (ncol(domain) == nvars) colnames(domain) <- varnames
  }
  suggestions <- NULL
...
print.summary.ga <- function(x, digits = getOption("digits"), ...) {
  ...
  cat(
    cli::rule(
      left = crayon::bold("Genetic Algorithm"),
  ...
  cat(paste(
    ...
    format(x$pcrossover, digits = digits),
    "\n"
  ))
  cat(paste(
    "Mutation probability  = ",
    format(x$pmutation, digits = digits),
    "\n"
  ))
  #
  ...
  cat(paste(
    ...
    format(x$fitness, digits = digits),
    "\n"
  ))
...
setGeneric(name = "parNames", def = function(object, ...) {
  standardGeneric("parNames")
})

setMethod("parNames", "ga", function(object, ...) {
  names <- object@names
  nvars <- ncol(object@population)
  if (length(names) == 0) {
